...
const ContactSalesTag = '[class="btn-mktg btn-subtle-mktg"]'
    ...
    describe('webdriver.io page', () => {
        ...
        it('hometask3-4 Scroll to Twitter logo on the footer', async () => {
            const elemTwitter = await $(TwitterelemTag);
            await elemTwitter.scrollIntoView();
            await browser.pause(2000) 
        });
        it('hometask3-5 Verify  Twitter logo on the footer is clickable', async () => {
            const elemTwitter = await $(TwitterelemTag);
            await elemTwitter.click()
            await browser.pause(2000)  
        })
        it('hometask3-6 Verify the "Contact Sales" button  is visible', async () => {
            await browser.url(homeUrl);
            await browser.pause(2000) 
            let ContactSales = await $(ContactSalesTag).isDisplayed();
        
        })
         it('hometask3-7 Verify the "Contact Sales" button is clickable', async () => {
